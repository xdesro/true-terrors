...
// gsap.set('.home-hero .segment--first', {
//     yPercent:-100
// })
...
const navItemsToReveal = homeHero.querySelectorAll('.home-nav__meta-title, .home-nav__list-item')
...
headerTl.from(accentImage, {
    opacity: 0,
    height: 0
}, '<-=.2')
headerTl.add(thirdSegmentTl, "<+=.1");
headerTl.from(marquee, {
    opacity: 0,
    duration: .4
},"<+=.1")
headerTl.from(warning, {
    opacity: 0,
    height: 0,
    y: '0'
}, '<')

headerTl.from(navItemsToReveal, {
x: -20,
opacity: 0,
stagger: .05
}, '<')

headerTl.from(description, {
    opacity: 0,
    duration :.8
}, '>')
